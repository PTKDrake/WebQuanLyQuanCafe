...
<head>
    <meta charset="UTF-8">
    <title>Coffee - Category list</title>
...
<div id="content" class="home">
    <div class="container">
        <h3 class="heading">Category<span>List</span></h3>
        <?php
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            $id = $_POST['id'] ?? '';
            if ($id != '') {
                if (removeCategory($id)) {
                    echo "<h3>Removed category</h3>";
                }
            }
        }
        ?>
        <table class="table table-bordered">
            ...
            <tr>
                <th>Name</th>
                <th>Description</th>
                <th><a class="btn btn-primary" href="category_create.php">Add</a></th>
            </tr>
            </thead>
            <tbody>
            <?php
            foreach (getCategories() as $category) {
                ...
                echo "<td><a class=\"btn btn-primary\" href=\"category_edit.php?id={$category[0]}\">Edit</a> <form action=\"category_list.php\" method=\"post\"><input name='id' hidden value='{$category[0]}'><button type='submit' class='btn btn-primary'>Remove</button></form></td>";
                echo '</tr>';
            }
            ?>
            </tbody>
